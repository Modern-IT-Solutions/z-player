...
class Youtube {
  ...
  Future<Set<ZStream>> streams(String id, {bool? isLive}) async {
    ...
    // get video response
    var response = await StreamController(YoutubeHttpClient())
        .getPlayerResponse(VideoId.fromString(id));
        ...
        : {
            ...
            .map((stream) {
              if (stream.videoCodec?.isNotEmpty == true) {
                ...
                if (stream.audioCodec?.isNotEmpty == true &&
    ...
    var expireDate = expire == null
        ? DateTime.now().add(Duration(hours: 1))
        : DateTime.fromMillisecondsSinceEpoch(int.parse(expire) * 1000);
    var diff = expireDate.difference(DateTime.now());
    remember(
        "zplayer_cache_video_$id", data.map((e) => e.toMap()).toList(), diff);
    return data;
  }
}
